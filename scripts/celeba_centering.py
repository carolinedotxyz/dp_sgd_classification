...
def load_subset_items(subset_root: str) -> List[Item]:
    ...
    if idx_csv and os.path.isfile(idx_csv):
        ...
        for _, r in df.iterrows():
            ...
            split = str(r.get("partition_name", r.get("split", "train")))
...
def load_landmarks(archive_dir: str) -> pd.DataFrame:
    ...
    df = pd.read_csv(path)
    req = ["image_id","lefteye_x","lefteye_y","righteye_x","righteye_y","nose_x","nose_y"]
    for c in req:
...
def compute_offsets(items: List[Item], lm_df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    lm_lookup = lm_df.set_index("image_id")[
...
def save_plots(df: pd.DataFrame, out_dir: str) -> None:
    ...
    plt.hist(df["radius"], bins=40, color="#4C78A8")
    plt.xlabel("distance from center (normalized)")
    plt.ylabel("count")
    plt.title("Face center distance distribution")
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "center_distance_hist.png"), dpi=200)
    plt.close()

    # 2D scatter/hex of dx, dy
    plt.figure(figsize=(5, 5))
    hb = plt.hexbin(df["dx"], df["dy"], gridsize=40, cmap="viridis")
    ...
    cb.set_label("count")
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "center_offset_hex.png"), dpi=200)
    plt.close()
...
def draw_outliers_grid(df: pd.DataFrame, out_png: str, k: int = 32, cols: int = 8) -> None:
    ...
    grid = Image.new("RGB", (cols * tile_w, rows * tile_h), (30, 30, 30))
    ...
    for i, (_, row) in enumerate(sel.iterrows()):
        try:
            with Image.open(row["path"]) as im:
                ...
                r = 4
                draw.ellipse((cx - r, cy - r, cx + r, cy + r), fill=(255, 50, 50))
                # annotate radius
                txt = f"{row['split'][:1]}/{row['class_name'][:2]} r={row['radius']:.3f}"
                draw.rectangle((0, 0, tile_w, 18), fill=(0, 0, 0, 128))
                try:
                    draw.text((4, 2), txt, fill=(255, 255, 255))
                except Exception:
                    pass
                r_idx = i // cols
                c_idx = i % cols
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    ...
    p.add_argument("--archive-dir", type=str, default="./data/celeba/archive", help="Path to CelebA archive (for landmarks CSV).")
    p.add_argument("--out-dir", type=str, default=None, help="Output directory for analysis artifacts.")
    p.add_argument("--k-outliers", type=int, default=32, help="Top-K most off-center examples to render.")
...
def main(argv: List[str]) -> int:
    ...
    out_dir = os.path.abspath(args.out_dir or os.path.join(subset_root, "centering_analysis"))
    ...
    draw_outliers_grid(df, os.path.join(out_dir, "outliers_grid.png"), k=int(args.k_outliers))
