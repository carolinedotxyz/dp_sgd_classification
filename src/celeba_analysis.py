...
from src.nb_display import ATTR_RENAME
...
def compute_attribute_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    # Local import from src to avoid scripts dependency
    from src.celeba_analysis import compute_balance_from_df  # type: ignore
...
def build_balance_comparison(dfo: pd.DataFrame, dfp: pd.DataFrame):
    ...
    rows = []
    from src.nb_display import _fmt_pct, _pp
    for s in splits:
